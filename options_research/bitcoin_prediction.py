"""
Bitcoin Prediction Model
========================
- Hourly data (faster feedback)
- 1 year of hourly data = ~8,000 samples
- Predict: Will price go UP or DOWN in next hour?
"""

import numpy as np
import pandas as pd
import yfinance as yf
import torch
import torch.nn as nn
from torch.utils.data import DataLoader, TensorDataset
from sklearn.preprocessing import StandardScaler
import warnings
warnings.filterwarnings('ignore')

# ============================================================
# BITCOIN FEATURES
# ============================================================

def get_bitcoin_data(period="1y", interval="1h"):
    """Get Bitcoin hourly data."""
    btc = yf.download("BTC-USD", period=period, interval=interval, progress=False)
    
    if isinstance(btc.columns, pd.MultiIndex):
        btc.columns = btc.columns.get_level_values(0)
    
    return btc


def create_features(btc, lookback=24):
    """
    Create features for Bitcoin.
    Lookback = number of hours to look back
    """
    df = btc.copy()
    
    # Returns at different horizons
    for w in [1, 2, 4, 8, 12, 24]:
        df[f'return_{w}h'] = df['Close'].pct_change(w)
    
    # Moving averages
    for w in [6, 12, 24, 48, 168]:  # 6h, 12h, 24h, 2d, 1w
        df[f'sma_{w}'] = df['Close'].rolling(w).mean()
        df[f'ema_{w}'] = df['Close'].ewm(span=w).mean()
    
    # Price relative to MAs
    df['price_sma24'] = df['Close'] / df['sma_24']
    df['price_sma168'] = df['Close'] / df['sma_168']
    df['sma24_sma168'] = df['sma_24'] / df['sma_168']
    
    # RSI (hourly)
    for w in [8, 24, 72]:
        delta = df['Close'].diff()
        gain = delta.where(delta > 0, 0).rolling(w).mean()
        loss = (-delta.where(delta < 0, 0)).rolling(w).mean()
        df[f'rsi_{w}'] = 100 - (100 / (1 + gain / loss))
    
    # MACD
    ema12 = df['Close'].ewm(span=12).mean()
    ema26 = df['Close'].ewm(span=26).mean()
    df['macd'] = ema12 - ema26
    df['macd_signal'] = df['macd'].ewm(span=9).mean()
    df['macd_hist'] = df['macd'] - df['macd_signal']
    
    # Bollinger Bands
    bb_mid = df['Close'].rolling(24).mean()
    bb_std = df['Close'].rolling(24).std()
    df['bb_upper'] = bb_mid + 2 * bb_std
    df['bb_lower'] = bb_mid - 2 * bb_std
    df['bb_position'] = (df['Close'] - df['bb_lower']) / (df['bb_upper'] - df['bb_lower'] + 0.001)
    
    # Volume
    df['volume_ma24'] = df['Volume'].rolling(24).mean()
    df['volume_ratio'] = df['Volume'] / df['volume_ma24']
    
    # Volatility
    df['vol_24h'] = df['return_1h'].rolling(24).std() * np.sqrt(24)
    df['vol_168h'] = df['return_1h'].rolling(168).std() * np.sqrt(168)
    
    # Hour of day (cyclical encoding)
    df['hour'] = df.index.hour
    df['hour_sin'] = np.sin(2 * np.pi * df['hour'] / 24)
    df['hour_cos'] = np.cos(2 * np.pi * df['hour'] / 24)
    
    # Lagged returns
    for lag in [1, 2, 3, 6, 12]:
        df[f'return_1h_lag{lag}'] = df['return_1h'].shift(lag)
    
    # Target: Next hour return (UP = 1, DOWN = 0)
    df['target'] = (df['Close'].shift(-1) > df['Close']).astype(int)
    
    df = df.dropna()
    
    # Features
    exclude = ['target', 'Close', 'Open', 'High', 'Low', 'Volume', 'Adj Close', 'hour']
    features = [c for c in df.columns if c not in exclude]
    
    return df[features], df['target'], df['Close']


# ============================================================
# MODEL
# ============================================================

class BitcoinPredictor(nn.Module):
    def __init__(self, input_dim):
        super().__init__()
        self.net = nn.Sequential(
            nn.Linear(input_dim, 256),
            nn.LayerNorm(256),
            nn.ReLU(),
            nn.Dropout(0.3),
            nn.Linear(256, 128),
            nn.LayerNorm(128),
            nn.ReLU(),
            nn.Dropout(0.3),
            nn.Linear(128, 64),
            nn.ReLU(),
            nn.Dropout(0.2),
            nn.Linear(64, 1)
        )
    
    def forward(self, x):
        return self.net(x)


# ============================================================
# MAIN
# ============================================================

if __name__ == "__main__":
    print("="*60)
    print("BITCOIN PREDICTION MODEL")
    print("="*60)
    
    # Get data
    print("\nDownloading Bitcoin data...")
    btc = get_bitcoin_data("2y", "1h")
    print(f"Got {len(btc)} hourly data points")
    
    # Create features
    print("\nCreating features...")
    X, y, prices = create_features(btc)
    print(f"Features: {X.shape[1]}")
    print(f"Samples: {len(X)}")
    print(f"Class: UP={y.sum()} ({y.mean()*100:.1f}%)")
    
    # Time-based split: 80% train, 20% test
    split = int(len(X) * 0.8)
    
    X_train, X_test = X.iloc[:split], X.iloc[split:]
    y_train, y_test = y.iloc[:split], y.iloc[split:]
    
    # Scale
    scaler = StandardScaler()
    X_train_scaled = scaler.fit_transform(X_train)
    X_test_scaled = scaler.transform(X_test)
    
    print(f"\nTrain: {len(X_train)}, Test: {len(X_test)}")
    
    # Train
    print("\n" + "="*60)
    print("TRAINING MODEL")
    print("="*60)
    
    train_ds = TensorDataset(
        torch.FloatTensor(X_train_scaled),
        torch.FloatTensor(y_train.values).reshape(-1, 1)
    )
    train_loader = DataLoader(train_ds, 256, shuffle=True, drop_last=True)
    
    device = 'cuda' if torch.cuda.is_available() else 'cpu'
    print(f"Device: {device}")
    
    model = BitcoinPredictor(X_train.shape[1]).to(device)
    optimizer = torch.optim.AdamW(model.parameters(), lr=1e-4, weight_decay=1e-4)
    criterion = nn.BCEWithLogitsLoss()
    scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, 'max', factor=0.5, patience=10)
    
    best_acc = 0
    best_state = None
    
    for epoch in range(80):
        model.train()
        for x, yb in train_loader:
            x, yb = x.to(device), yb.to(device)
            optimizer.zero_grad()
            loss = criterion(model(x), yb)
            loss.backward()
            torch.nn.utils.clip_grad_norm_(model.parameters(), 1.0)
            optimizer.step()
        
        # Validate
        model.eval()
        with torch.no_grad():
            preds = (torch.sigmoid(model(torch.FloatTensor(X_test_scaled).to(device))) > 0.5).float()
            acc = (preds.squeeze() == torch.FloatTensor(y_test.values).to(device)).float().mean().item()
        
        scheduler.step(acc)
        
        if acc > best_acc:
            best_acc = acc
            best_state = model.state_dict().copy()
        
        if (epoch + 1) % 20 == 0:
            print(f"Epoch {epoch+1}: Test Acc = {acc:.2%}")
    
    model.load_state_dict(best_state)
    print(f"\nBest Test Accuracy: {best_acc:.2%}")
    
    # Backtest
    print("\n" + "="*60)
    print("BACKTEST")
    print("="*60)
    
    model.eval()
    with torch.no_grad():
        probs = torch.sigmoid(model(torch.FloatTensor(X_test_scaled).to(device))).cpu().numpy().flatten()
    
    predictions = (probs > 0.5).astype(int)
    
    # Returns
    returns = X_test['return_1h'].values
    
    # Strategy: long when UP, short when DOWN
    strategy_returns = returns * (predictions * 2 - 1)
    
    # Costs
    costs = 0.001  # 0.1% per trade (generous for crypto)
    strategy_returns = strategy_returns - costs * np.abs(np.diff(np.concatenate([[0], predictions])))
    
    # Cumulative
    strat_cum = (1 + strategy_returns).cumprod()
    bh_cum = (1 + returns).cumprod()
    
    strat_ret = strat_cum[-1] - 1
    bh_ret = bh_cum[-1] - 1
    
    strat_sharpe = strategy_returns.mean() / (strategy_returns.std() + 1e-8) * np.sqrt(24*365)
    bh_sharpe = returns.mean() / (returns.std() + 1e-8) * np.sqrt(24*365)
    
    print(f"\nTest Period: {len(X_test)} hours")
    print(f"ML Strategy: {strat_ret*100:+6.2f}% (Sharpe: {strat_sharpe:.2f})")
    print(f"Buy & Hold: {bh_ret*100:+6.2f}% (Sharpe: {bh_sharpe:.2f})")
    
    # Hourly breakdown
    hourly_return = (strat_cum[-1] ** (1/len(strat_cum)) - 1) * 100
    print(f"\nHourly return: {hourly_return:.4f}%")
    
    print("\n✅ Done!")